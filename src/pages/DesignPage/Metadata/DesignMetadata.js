...
export const DesignMetadata = forwardRef((props, ref) => {
    ...
    const handleChangeKeywords = (keywords, type, targetKeyword) => {
        if(type === 'add') socket.emit('add-design-keyword', { designId: design._id, targetKeyword});
        else if (type === 'remove') socket.emit('remove-design-keyword', { designId: design._id, targetKeyword});
    };
